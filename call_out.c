...
/*
 * Free a call out structure.	释放结构体
 */
...
void call_out()
{
    ...
    real_time = get_current_time();		/* 记录当前时间 */
    ...
    save_context(&econ);
    while (1) {
        tm = current_time & (CALLOUT_CYCLE_SIZE - 1);	/*  cur&(32-1) */
        DBG(("   slot %i", tm));
        ...
        /* Ok, no more scheduled call_outs for current_time 最近没有什么调用可以处理了 */
