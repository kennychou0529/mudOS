...
void init_binaries(int  argc, char **  argv)
{
    ...
    if (do_stat(argv[0], &st, driver_name) != -1) {
        ...
        driver_id = current_time;		/* 用时间作为driver_id不错 */
...
FILE *crdir_fopen(char *  file_name)
{
     ...
     */
    if ((ret = fopen(file_name, "wb")) != NULL) {	/* 可能文件已存在了，不用创建了 */
        return ret;
    }
    p = file_name;
    ...
    while (*p && (p = (char *) strchr(p, '/'))) {
        *p = '\0';
        if (stat(file_name, &st) == -1) {
            /* make this dir */
            if (OS_mkdir(file_name, 0770) == -1) {	/* 如果没有找到，就创建文件夹 */
                *p = '/';
                return (FILE *) 0;
            }
        }
    ...
    return fopen(file_name, "wb");	/* 文件夹都创建好了之后就可以打开了 */
}
/* crdir_fopen() */
