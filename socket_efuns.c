 ...
 */
int socket_bind(int  fd, int  port, char *  addr)
{
    int len;
 ...
 */
int socket_connect(int  fd, char *  name, svalue_t *  read_callback, svalue_t *  write_callback)
{
    if (fd < 0 || fd >= max_lpc_socks)
	return EEFDRANGE;
    if (lpc_socks[fd].state == STATE_CLOSED ||
 ...
 */
int socket_write(int  fd, svalue_t *  message, char *  name)
{
    int len, off;
    char *buf, *p;
    struct sockaddr_in sin;
