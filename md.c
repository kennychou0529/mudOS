...
int totals[MAX_CATEGORY];		/* 这两个又是干嘛用的？大小为130 */
int blocks[MAX_CATEGORY];

static char *sources[] = {		/* 这堆是什么鬼 */
...
static md_node_t **table;			/* 这是个表！！！哈希表 */
unsigned int total_malloced = 0L;
unsigned int hiwater = 0L;		/* hiwater就像是申请过的最多内存字节数量 */	
...
void MDinit()
{
    ...
    table = (md_node_t **) calloc(MD_TABLE_SIZE, sizeof(md_node_t *));	/* 分配MD_TABLE_SIZE块这么大的连续内存 */
    for (j = 0; j < MAX_CATEGORY; j++) {
        totals[j] = 0;			/* 全局变量 */
    }
}

/* 将一个节点以哈希的方式存在table中,并不涉及申请内存，别给误导了 */
void MDmalloc(md_node_t *  node, int  size, int  tag, char *  desc)
{
    unsigned int h;
    static int count = 0;

    if (!size) {		/* size是node后面的字节数量，若为0就没有意义了 */
...
#endif
#ifdef DEBUGMALLOC_EXTENSIONS	/* 可以不用鸟 */
