...
int inet_packets = 0;
int inet_volume = 0;
interactive_t **all_users = 0;	/* 用户存在这 */
int max_users = 0;				/* 肯定在哪里设置了 */
...
/*
 * Initialize new user connection socket.	初始化监听用户请求的socket
 */
void init_user_conn()
{
    ...
    int optval;
    int i;
    int have_fd6;		/* 这个6是什么意思？ */
    int fd6_which;

    /* Check for fd #6 open as a valid socket 默认以sockfd=6 */
    optval = 1;
    have_fd6 = (setsockopt(6, SOL_SOCKET, SO_REUSEADDR, (char *)&optval, sizeof(optval)) == 0);	/* 测试一下是否被占用 */

    for (i=0; i < 5; i++) {		/* 为什么这个时候还需要external port？ */
#ifdef F_NETWORK_STATS
...
#endif
        if (!external_port[i].port) {		/* 这是在设置什么？ */
            if (!have_fd6) continue;		/* 保证fd6只被用一次 */
            fd6_which = i;
            have_fd6 = 0;					/* fd6用在此端口，下面没有fd6了 */
            ...
            external_port[i].kind = FD6_KIND;	/* #fd6 kind : telnet */
            external_port[i].port = FD6_PORT;	/* port=23? */
            external_port[i].fd = 6;
        } else {
            /*
             * create socket of proper type.	申请了个socketfd?
             */
            ...
            /*
            * enable local address reuse.  设置socket参数
            */
            optval = 1;
            ...
            /*
             * fill in socket address information.	填充其他协议信息
             */
        ...
        /*
         * get socket name.	尝试获取socket名，检查错误？
         */
        sin_len = sizeof(sin);
        ...
        /*
         * listen on socket for connections.	开始监听端口
         */
    ...
    /*
     * register signal handler for SIGPIPE. 注册PIPE信号处理
     */
...
INLINE void process_io()
{
    ...
    for (i = 0; i < 5; i++) {
        if (!external_port[i].port) continue;
        if (FD_ISSET(external_port[i].fd, &readmask)) {	//找找看哪个是可读的
            debug(connections, ("process_io: NEW_USER\n"));
            new_user_handler(i);	/* 有人来了就处理他的请求 */
        }
    }
    /*
 ...
 * the user is connected.	处理用户请求telnet。。。。
 */
static void new_user_handler(int  which)
{
     ...
     * do it for everyone
     */
    if (set_socket_nonblocking(new_socket_fd, 1) == -1) {	/* 先设置非阻塞先 */
    ...
    for (i = 0; i < max_users; i++)
        if (!all_users[i]) break;		/* 找个好的洞洞装用户 */

    if (i == max_users) {
        if (all_users) {
            all_users = RESIZE(all_users, max_users + 10, interactive_t *,	/* 用户上限还是动态增长呢？ */
